<div class="w-full h-full px-2 relative">    
    <div class="hidden form absolute top-0 right-0 left-0 bottom-0 bg-gray-500 bg-opacity-30">
        <div class="border bg-gray-100 rounded-lg shadow p-4 mt-4 w-3/5 mx-auto">
            <div class="text-md font-bold">
                Ajouter / Modifier Catégorie d'appartement
            </div>
            <div class="my-4">
                <div class="text-xs mb-2">
                    Catégorie :
                </div>
                <input type="text" name="" value="fff" id="appartement_category" class="py-2 px-3 border border-gray-200 rounded">
    ...
    <div class="flex items-center justify-between">
        <div class="text-2xl">Catégorie d'appartements</div>
...
<script>
    $(document).ready(function(){
        ...
        $('.ajouter').on('click', function(){
            $("#id").val("")
            $("#appartement_category").val("")
            $('#is_default').prop('checked', false)
            $('.form').removeClass('hidden')
            $("#appartement_category").focus()
        })

        /** Edit */
        $('.modifier').on('click', function(){
            var appartement_category = $(this).data('appartement_category')
            var id = $(this).data('id')
            var is_default = $(this).data('is_default')
            $("#id").val(id)
            $("#appartement_category").val(appartement_category)
            if(is_default){
        ...
        $('.enregistrer').on('click', function(){
			var appartement_category = $('#appartement_category').val();
			var is_default = $('#is_default').prop("checked");

			var data = {
				'controler'		:	'Propriete_Category',
				'function'		:	'Store',
				'params'		:	{
					'propriete_category'		:	appartement_category,
